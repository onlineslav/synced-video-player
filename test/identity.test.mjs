...
test('a username is the chosen handle plus a tag hashed from the handle and key', async () => {
  ...
  const a = await createIdentity('moviefan', keys)
  const b = await createIdentity('moviefan')
  assert.match(a.username, USERNAME)
  assert.ok(a.username.startsWith('moviefan#'))
  assert.equal(await usernameFor(keys.publicKey, 'moviefan'), a.username, 'the welcome screen preview matches')
  assert.notEqual(a.username, b.username, 'same handle, different people')
  assert.notEqual((await createIdentity('moviefan2', keys)).username.split('#')[1], a.username.split('#')[1], 'tag depends on the handle')
...
test('normalizeHandle', () => {
  assert.equal(normalizeHandle('  Moviefan_D.  '), 'moviefan_d.')
...
test('normalizeUsername accepts sloppy typing and rejects non-usernames', () => {
  assert.equal(normalizeUsername(' Moviefan # K7QM X3PA '), 'moviefan#k7qm-x3pa')
  assert.equal(normalizeUsername('moviefan#k7qm-x3pI'), 'moviefan#k7qm-x3p1', 'I and L read as 1')
  assert.equal(normalizeUsername('moviefan#k7qm-x3pO'), 'moviefan#k7qm-x3p0', 'O reads as 0')
  assert.equal(normalizeUsername('moviefan#k7qm-x3pu'), null, 'u is not in the alphabet')
  assert.equal(normalizeUsername('moviefan#k7qm'), null)
