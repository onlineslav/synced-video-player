...
- **Identity** (`renderer/identity.mjs`): each install keeps an ECDSA P-256 key pair in `localStorage`. The username is a 12-character Crockford base32 hash of the public key, so it can be checked against the key. A new key pair means a new username. The display name (`renderer/profile.mjs`) is separate, editable on the home screen or in your own room-list row, and cleaned with `cleanDisplayName` both when typed and when received.
