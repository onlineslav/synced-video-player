...
- **Identity** (`renderer/identity.mjs`): each install keeps an ECDSA P-256 key pair in `localStorage`. The username is a handle the person picks plus a tag, `handle#xxxx-xxxx`: 8 Crockford base32 characters hashed from the handle and public key, so it can be checked against the key (faking one takes ~2^40 key pairs). A new install, or an identity saved before handles existed, boots into the welcome screen (`showWelcome`) and only reaches home once both a username and display name are saved; "Change" on home reopens it with a new key pair. The display name (`renderer/profile.mjs`) is separate, editable on the home screen or in your own room-list row, and cleaned with `cleanDisplayName` both when typed and when received.
